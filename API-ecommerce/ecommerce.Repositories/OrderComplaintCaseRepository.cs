...
{
    public class OrderComplaintCaseRepository : Repository<OrderComplaintCase>, IOrderComplaintCase
    {

        private readonly ApplicationDbContext _db;
        public OrderComplaintCaseRepository(ApplicationDbContext db) : base(db)
        {
            _db = db;
        }

        public async Task<POJO> Save(OrderComplaintCase entity)
        {
            POJO model = new POJO();
            if (entity.id == 0)
            {
                try
                {
                    await _db.OrderComplaintCases.AddAsync(entity);
            ...
            {
                OrderComplaintCase _Entity = await _db.OrderComplaintCases.FindAsync(entity.id);
